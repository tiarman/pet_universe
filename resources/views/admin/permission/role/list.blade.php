              ...
              @if(\App\Helper\CustomHelper::canView('Create Role', 'Super Admin'))
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('role.create') }}" class="brn btn-success btn-sm">Create new Role</a>
                  </div>
                </div>
              @endif
                      ...
                      <td class="center hidden-phone p-1" width="100">
                        @if(in_array($val->id, [1,2,3]))
                          @if(\App\Helper\CustomHelper::canView('Manage Role', 'Super Admin'))
                            <a href="#" title="Edit" style="cursor: not-allowed"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Role', 'Super Admin'))
                            <span class="btn btn-sm btn-danger" title="Delete" style="cursor: not-allowed"><i class="fa fa-trash-o"></i></span>
                          @endif
                        @else
                          @if(\App\Helper\CustomHelper::canView('Manage Role', 'Super Admin'))
                            <a href="{{ route('role.manage', [$val->id]) }}" title="Edit"
                               class="btn btn-sm btn-success"> <i class="fa fa-edit"></i> </a>
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Role', 'Super Admin'))
                            <span title="Delete" class="btn btn-sm btn-danger btn-delete delete_{{ $val->id }}" style="cursor: pointer"
