            ...
            <div class="text-center">
                <span class="btn btn-sm btn-primary btn-add" style="cursor: pointer"
                 title="Delete"><i class="fa fa-plus-circle">
                    <span> Add Sub Category <span class="pull-right"></span> </span></i></span>
            </div>
                    ...
                    <div class="row">
                      <div class="col-sm-6">
                        <div class="form-group">
                          ...
                          <input type="text" name="subcategory_name" placeholder="Enter sub category name" value="{{ old('subcategory_name') }}"
                                 class="form-control @error('subcategory_name') is-invalid @enderror" required>
                      ...
                      <div class="col-sm-6">
                        <div class="form-group">
                          ...
                          <input type="text" name="subcategory_slug" placeholder="Enter sub category slug" value="{{ old('subcategory_slug') }}"
                                 class="form-control @error('subcategory_slug') is-invalid @enderror" required>
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                @foreach($data as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->subcategory_name }}</td>
                    <td class="p-1">{{ $val->subcategory_slug }}</td>
                    ...
                    @if(\App\Helper\CustomHelper::canView('Manage User|Delete User', 'Super Admin'))
                      <td class="center hidden-phone p-1" width="100">
                        @if(\App\Helper\CustomHelper::canView('Manage User', 'Super Admin'))
                          <a href="{{ route('subcategory.manage', $val->id) }}" class="btn btn-sm btn-success" title="Edit"> <i
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('ajax.update.subcategory.status') }}",
          method: "post",
          dataType: "html",
